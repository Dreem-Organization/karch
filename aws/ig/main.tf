...
FILEDUMP
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      ${var.nodeup-url-env} AWS_SDK_LOAD_CONFIG=1 AWS_PROFILE=${var.aws-profile} ${var.kops-cmd} --state=s3://${var.kops-state-bucket} create -f ${path.module}/${var.cluster-name}-${var.name}-ig-spec.yml
