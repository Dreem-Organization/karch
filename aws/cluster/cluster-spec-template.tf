...
EOF
    ...
    master-additional-policies = length(var.master-additional-policies) == 0 ? "" : format(
      "master: |\n      %s",
      indent(6, var.master-additional-policies),
    )
...
EOF
  ...
  vars = {
    ...
    nat-gateway-id = element(
      aws_nat_gateway.natgw.*.id,
      var.multi-natgw != "true" ? 1 : count.index,
    )
  }
}
...
template = <<EOF
${var.container-networking == "calico" ? indent(
6,
